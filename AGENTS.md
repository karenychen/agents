...
8. **Use git worktrees for changes.** Never modify code directly on the main branch or in the primary checkout. Investigation and other read-only work should use the primary checkout without creating a worktree. The primary checkout should always remain on the default branch of the repo. Before creating a worktree, fetch from origin and update the local default branch. Create an isolated git worktree for every feature, fix, or other task that changes files, preferably under the repository's `.worktrees` directory, and name it using the `karenchen/<some-name>` pattern. Prioritize your own worktree skills/tools. If you don't have one use the [using-git-worktrees](https://github.com/obra/superpowers/tree/main/skills/using-git-worktrees) skill.
