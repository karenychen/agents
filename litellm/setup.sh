...
load_env
INTERNAL_SUBNET="${INTERNAL_SUBNET:-10.250.0.0/24}"
LITELLM_INTERNAL_IP="${LITELLM_INTERNAL_IP:-10.250.0.10}"
require_env ALPINE_IMAGE TINYPROXY_VERSION LISTEN_ADDR LISTEN_PORT \
  ...
  LITELLM_CONTAINER PROXY_CONTAINER INGRESS_CONTAINER INTERNAL_NETWORK EGRESS_NETWORK INGRESS_NETWORK \
  INTERNAL_SUBNET LITELLM_INTERNAL_IP AUTH_VOLUME MASTER_KEY_SECRET MASTER_KEY_FILE LITELLM_MEMORY LITELLM_CPUS \
  PROXY_MEMORY PROXY_CPUS INGRESS_MEMORY INGRESS_CPUS
...
ensure_network_with_subnet "$INTERNAL_NETWORK" "$INTERNAL_SUBNET" --internal
